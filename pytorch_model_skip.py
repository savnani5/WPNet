...
class UpSample_new(nn.Sequential):
    def __init__(self, skip_input, output_features):
        ...
        self.convAx = nn.Conv2d(skip_input, output_features, kernel_size=3, stride=1, padding=1)
        self.leakyreluA = nn.LeakyReLU(0.2)
        self.convBx = nn.Conv2d(output_features, output_features, kernel_size=3, stride=1, padding=1)
        self.leakyreluB = nn.LeakyReLU(0.2)

    def forward(self, x, concat_with):
        up_x = F.interpolate(x, size=[concat_with.size(2), concat_with.size(3)], mode='bilinear', align_corners=True)
        return self.leakyreluB( self.convBx( self.convAx( torch.cat([up_x, concat_with], dim=1)  ) )  )
...
class Decoder(nn.Module):
    def __init__(self, num_features=1664, decoder_width = 1.0):
        ...
        self.up1_new = UpSample_new(skip_input=features//1 + features//2, output_features=features//2)
        self.up2_new = UpSample_new(skip_input=features//2 + features//2,  output_features=features//4)
        self.up3_new = UpSample_new(skip_input=features//4 + features//4,  output_features=features//8)
        self.up4_new = UpSample_new(skip_input=features//8 + features//8,  output_features=features//16)
        ...
        self.skip13=res_block(in_channels=features//4, out_channels=features//8)
        self.skip21=res_block(in_channels=64, out_channels=features//4)
        self.skip22=res_block(in_channels=features//4, out_channels=features//4)
        self.skip31=res_block(in_channels=128, out_channels=features//2)
        self.skip41=res_block(in_channels=256, out_channels=features//2)
